...
function loadList() {
    ...
    xhr.addEventListener("readystatechange", function () {
        ...
        for (i=0; i< this.response.length;i++){
            ...
            name.textContent = coinData.fullName;
            prof.textContent = coinData.price_usd;
            info.textContent = coinData.shortInfo;
...
function sendForm() {
    ...
    $form.on("submit", function (e) {
        ...
        if ($btnCreate.get(0).on){
            ...
            xhr.open("put", "coin?id=" + $btnCreate.get(0).dataId);
        }
...
function removehendler() {
    $table.on("click",".btn-remove", function () {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open("delete", "coin?id=" + $remove.data.id);
        xhr.send();
        xhr.addEventListener("readystatechange", function () {
...
function edithendler() {
    $table.on("click",".btn-edit", function () {
        ...
        $("#price_usd").get(0).value = $edit.data.price_usd;
...
function writeDataFromForm() {
    ...
    data.price_usd = $("#price_usd").get(0).value;
